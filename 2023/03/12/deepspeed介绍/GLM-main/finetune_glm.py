...
def finetune(args, train_valid_datasets_provider, model_kwargs, forward_step=finetune_forward_step,
    ...
    定义一个函数finetune，接受四个参数：
    args（命令行参数），
    train_valid_datasets_provider（训练和验证数据集提供者），
    model_kwargs（模型参数），
    forward_step（前向传播步骤）"""
    ...
    global tokenizer
    timers = Timers()
    # 调用prepare_tokenizer函数，使用args参数来准备分词器tokenizer
    tokenizer = prepare_tokenizer(args)
    # 将pretrain_glm模块中的tokenizer属性赋值为tokenizer
    pretrain_glm.tokenizer = tokenizer
     # 如果args有save属性，则将其赋值为args.save和args.experiment_name拼接后的路径
    if args.save:
        args.save = os.path.join(args.save, args.experiment_name)
    # Train and validation data loaders.
    # 创建一个timers对象，并调用其start方法，开始计时训练和验证数据加载器的时间
    timers('train/valid/test dataset/dataloder').start()
     # 初始化四个变量为None
    train_dataloader, valid_dataloader = None, None
    train_block_dataloader, valid_block_dataloader = None, None
    # 如果train_valid_datasets_provider不是None，并且args有epochs属性且大于0，说明需要进行训练和验证
    if train_valid_datasets_provider is not None and args.epochs > 0:
         # 如果mpu模块中的get_model_parallel_rank函数返回值为0，说明是模型并行的主进程
        if mpu.get_model_parallel_rank() == 0:
            # 调用train_valid_datasets_provider函数，使用args和tokenizer参数来获取训练和验证数据集
            train_dataset, valid_dataset = train_valid_datasets_provider(args, tokenizer)
            # 调用_build_train_valid_dataloaders函数，使用train_dataset，valid_dataset和args参数来构建训练和验证数据加载器
            train_dataloader, valid_dataloader = _build_train_valid_dataloaders(train_dataset, valid_dataset, args)
            # 如果args有no_validation属性，则将valid_dataloader赋值为None，表示不进行验证
            if args.no_validation:
                valid_dataloader = None
            # 创建一个torch.cuda.LongTensor对象，使用train_dataloader的长度作为元素，赋值给train_iters变量
            train_iters = torch.cuda.LongTensor([len(train_dataloader)])
        else:
            # 否则，如果不是主进程，则创建一个torch.cuda.LongTensor对象，使用0作为元素，赋值给train_iters变量
            train_iters = torch.cuda.LongTensor([0])
        # 调用torch.distributed模块中的broadcast函数，在mpu.get_model_parallel_group()组内广播train_iters变量，
        # 从mpu.get_model_parallel_src_rank()进程发送到其他进程
        torch.distributed.broadcast(train_iters, mpu.get_model_parallel_src_rank(),
                                    group=mpu.get_model_parallel_group())
        if mpu.get_model_parallel_rank() != 0:
            # 如果不是模型并行的主进程，则将args的train_iters_per_epoch属性赋值为train_iters的第一个元素，
            # 将args的train_iters属性赋值为args.epochs乘以args.train_iters_per_epoch
            args.train_iters_per_epoch = train_iters[0].item()
            args.train_iters = args.epochs * args.train_iters_per_epoch
            # 如果不是模型并行的主进程，则创建一个FakeDataloader对象，使用args.train_iters_per_epoch作为参数，赋值给train_dataloader变量，
            # 如果args有no_validation属性，则将valid_dataloader赋值为None，否则创建一个FakeDataloader对象，使用None作为参数，赋值给valid_dataloader变量
            train_dataloader = FakeDataloader(args.train_iters_per_epoch)
            if args.no_validation:
                valid_dataloader = None
            else:
                valid_dataloader = FakeDataloader(None)
        # 如果args有block_lm_ratio属性且大于0.0，说明需要进行块级语言模型训练
        if args.block_lm_ratio > 0.0:
            # 如果是模型并行的主进程, 调用train_valid_datasets_provider函数，使用args，tokenizer和block_lm=True参数来获取训练和验证块数据集
            if mpu.get_model_parallel_rank() == 0:
    ...
    # 调用timers对象的stop方法，停止计时训练和验证数据加载器的时间
    timers('callback function').stop()

    # Build model, optimizer and learning rate scheduler.
    # 创建一个timers对象，并调用其start方法，开始计时模型和优化器的时间
    timers('model and optimizer').start()
    # 调用pretrain_glm模块中的GLMModel类，使用model_kwargs参数来创建一个模型对象，赋值给model变量
